...
if [ "${API_URL}X" == "X" ]; then
    echo "=> Setting API URL to default"
    export API_URL="http://127.0.0.1:8086"
fi
...
if [ -n "${PRE_CREATE_DB}" ]; then
    ...
    if [ -f "/data/.pre_db_created" ]; then
        ...
        do
            ...
            echo "curl -v -s -k -X POST -d \"{\"name\":\"${x}\"}\" \"${API_URL}/db?u=root&p=${PASS}\""
            curl -v -s -k -X POST -d "{\"name\":\"${x}\"}" "${API_URL}/db?u=root&p=${PASS}"
        done
        echo ""
